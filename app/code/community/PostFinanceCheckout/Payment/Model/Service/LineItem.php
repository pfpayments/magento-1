...
class PostFinanceCheckout_Payment_Model_Service_LineItem extends PostFinanceCheckout_Payment_Model_Service_Abstract
{
    ...
    protected function getInvoiceShippingLineItem(Mage_Sales_Model_Order_Invoice $invoice)
    {
        if ($invoice->getShippingAmount() > 0) {
            ...
            if (Mage::getStoreConfig('postfinancecheckout_payment/line_item/overwrite_shipping_description', $invoice->getStore())) {
                $lineItem->setName(Mage::getStoreConfig('postfinancecheckout_payment/line_item/custom_shipping_description', $invoice->getStore()));
            } else {
                $lineItem->setName(
                    $invoice->getOrder()
    ...
    protected function getShippingLineItem($entity)
    {
        ...
        if ($shippingInfo->getShippingAmount() > 0) {
            ...
            if (Mage::getStoreConfig('postfinancecheckout_payment/line_item/overwrite_shipping_description', $entity->getStore())) {
                $lineItem->setName(Mage::getStoreConfig('postfinancecheckout_payment/line_item/custom_shipping_description', $entity->getStore()));
            } else {
