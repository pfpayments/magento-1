...
class PostFinanceCheckout_Payment_Model_Service_PaymentMethodConfiguration extends PostFinanceCheckout_Payment_Model_Service_Abstract
{
    ...
    private function hasChanged(\PostFinanceCheckout\Sdk\Model\PaymentMethodConfiguration $configuration, PostFinanceCheckout_Payment_Model_Entity_PaymentMethodConfiguration $model)
    {
    ...
    public function synchronize()
    {
        ...
        foreach ($existingConfigurations as $existingConfiguration) {
            $existingConfiguration->setState(PostFinanceCheckout_Payment_Model_Entity_PaymentMethodConfiguration::STATE_HIDDEN);
        }

        foreach (Mage::app()->getWebsites() as $website) {
            $spaceId = $website->getConfig('postfinancecheckout_payment/general/space_id');
            if ($spaceId && ! in_array($spaceId, $spaceIds)) {
                $paymentMethodConfigurationService = new \PostFinanceCheckout\Sdk\Service\PaymentMethodConfigurationService($this->getHelper()->getApiClient());
                $configurations = $paymentMethodConfigurationService->search($spaceId, new \PostFinanceCheckout\Sdk\Model\EntityQuery());
    ...
    protected function createPaymentMethodModelClasses()
    {
        /* @var PostFinanceCheckout_Payment_Model_Resource_PaymentMethodConfiguration_Collection $collection */
        $collection = Mage::getModel('postfinancecheckout_payment/entity_paymentMethodConfiguration')->getCollection();
        $generationDir = $this->getHelper()->getGenerationDirectoryPath() . DS . 'PostFinanceCheckout' . DS . 'Payment' . DS . 'Model';
        if (! file_exists($generationDir)) {
            mkdir($generationDir, 0777, true);
        }

        $classTemplate = file_get_contents(Mage::getModuleDir('', 'PostFinanceCheckout_Payment') . DS . 'Model' . DS . 'Payment' . DS . 'Method' . DS . 'Template.php.tpl');
