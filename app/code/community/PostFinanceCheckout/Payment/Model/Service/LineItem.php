...
/**
 * This service provides functions to convert Magento quote and order items into PostFinance Checkout API line items.
 */
class PostFinanceCheckout_Payment_Model_Service_LineItem extends PostFinanceCheckout_Payment_Model_Service_Abstract
{
    ...
    public function collectInvoiceLineItems(Mage_Sales_Model_Order_Invoice $invoice, $amount)
    {
        ...
        Mage::dispatchEvent('postfinancecheckout_payment_convert_invoice_line_items', array(
            'result' => $result,
            'invoice' => $invoice
        ));
    ...
    protected function getInvoiceShippingLineItem(Mage_Sales_Model_Order_Invoice $invoice)
    {
        if ($invoice->getShippingAmount() > 0) {
            ...
            if (Mage::getStoreConfig('postfinancecheckout_payment/line_item/overwrite_shipping_description',
                $invoice->getStore())) {
                $lineItem->setName(
                    Mage::getStoreConfig('postfinancecheckout_payment/line_item/custom_shipping_description',
                        $invoice->getStore()));
            } else {
    ...
    public function collectLineItems($entity)
    {
        ...
        $result = new StdClass();
        $result->items = $lineItems;
        Mage::dispatchEvent('postfinancecheckout_payment_convert_line_items', array(
            'result' => $result,
            'entity' => $entity
        ));
    ...
    protected function getProductLineItem($productItem, $currency)
    {
        ...
        Mage::dispatchEvent('postfinancecheckout_payment_convert_product_line_item', array(
            'result' => $result,
    ...
    protected function getDiscountLineItem($productItem, $currency)
    {
        ...
        if ($productItem->getDiscountAmount() != 0) {
            ...
            $lineItem->setType(\PostFinanceCheckout\Sdk\Model\LineItemType::DISCOUNT);
            $uniqueId = $productItem->getId();
            if ($productItem instanceof Mage_Sales_Model_Order_Item) {
                $uniqueId = $productItem->getQuoteItemId();
            } elseif ($productItem instanceof Mage_Sales_Model_Order_Invoice_Item) {
            ...
            $result = new StdClass();
            $result->item = $lineItem;
            Mage::dispatchEvent('postfinancecheckout_payment_convert_discount_line_item', array(
                'result' => $result,
                'entityItem' => $productItem,
                'currency' => $currency
            ));
    ...
    protected function getShippingLineItem($entity)
    {
        ...
        if ($shippingInfo->getShippingAmount() > 0) {
            ...
            $result = new StdClass();
            $result->item = $lineItem;
            Mage::dispatchEvent('postfinancecheckout_payment_convert_shipping_line_item', array(
                'result' => $result,
                'entity' => $entity
            ));
            return $this->cleanLineItem($result->item);
    ...
    protected function getFoomanSurchargeLineItem($entity)
    {
        if (Mage::helper('core')->isModuleEnabled('Fooman_Surcharge') && $entity->getFoomanSurchargeAmount() != 0) {
            ...
            $lineItem->setType(\PostFinanceCheckout\Sdk\Model\LineItemType::FEE);
     ...
     *
     * @return PostFinanceCheckout_Payment_Helper_LineItem
     */
    protected function getLineItemHelper()
    {
        return Mage::helper('postfinancecheckout_payment/lineItem');
    }
}