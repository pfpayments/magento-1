 ...
 */
MagePostFinanceCheckout.Checkout.Type.MageStoreOneStepCheckout = Class.create(
		...
		onOrderCreated: function(response) {
			if (response && response.status == 200 && response.responseText == 'OK') {
				if (this.getPaymentMethod(this.getPaymentMethodCode()).handler) {
					...
					location.href = MagePostFinanceCheckout.Checkout.paymentPageUrl + '&paymentMethodConfigurationId=' + this.getPaymentMethod(this.getPaymentMethodCode()).configurationId;
				}
...
MagePostFinanceCheckout.Checkout.type = MagePostFinanceCheckout.Checkout.Type.MageStoreOneStepCheckout;