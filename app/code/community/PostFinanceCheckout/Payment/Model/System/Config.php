...
class PostFinanceCheckout_Payment_Model_System_Config
{
    ...
    const SYSTEM_CACHE_ID = 'postfinancecheckout_system_config';

    const VALUES_CACHE_ID = 'postfinancecheckout_config_values';
        ...
        if (isset($parts[$websiteCode]) && ! empty($parts[$websiteCode])) {
            ...
            if ($spaceId) {
                $paymentMethodTemplate = file_get_contents(
                    Mage::getModuleDir('etc', 'PostFinanceCheckout_Payment') . DS . 'payment_method.system.xml.tpl');
    ...
    public function initConfigValues()
    {
        if (Mage::getConfig() instanceof PostFinanceCheckout_Payment_Model_Core_Config) {
            ...
            if (!$configLoaded) {
                ...
                $this->setConfigValue('postfinancecheckout/config_loaded', true);
        ...
        if (($cachedValues = Mage::app()->loadCache(self::VALUES_CACHE_ID))) {
            ...
            foreach ($collection as $paymentMethod) {
                ...
                if (isset($websiteMap[$paymentMethod->getSpaceId()])) {
                    ...
                    PostFinanceCheckout_Payment_Model_Entity_PaymentMethodConfiguration::STATE_ACTIVE ? 1 : 0;
    ...
    protected function getPaymentMethodTitle(
        PostFinanceCheckout_Payment_Model_Entity_PaymentMethodConfiguration $paymentMethod, $locale)
    {
    ...
    protected function isTableExists()
    {
        ...
        if ($connection) {
            return $connection->isTableExists(
                $resource->getTableName('postfinancecheckout_payment/payment_method_configuration'));
        } else {
            return false;
        }
    }
