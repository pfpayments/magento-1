...
class PostFinanceCheckout_Payment_Model_Webhook_DeliveryIndication extends PostFinanceCheckout_Payment_Model_Webhook_AbstractOrderRelated
{
    ...
    {
        $deliveryIndicationService = new \PostFinanceCheckout\Sdk\Service\DeliveryIndicationService(Mage::helper('postfinancecheckout_payment')->getApiClient());
    ...
    protected function review(Mage_Sales_Model_Order $order)
    {
        if ($order->getState() != Mage_Sales_Model_Order::STATE_PAYMENT_REVIEW) {
            $order->setState(Mage_Sales_Model_Order::STATE_PAYMENT_REVIEW, true, Mage::helper('postfinancecheckout_payment')->__('A manual decision about whether to accept the payment is required.'));
            $order->save();
        }
    }
