 ...
 */
class PostFinanceCheckout_Payment_Model_System_Config
{
    ...
    public function initSystemConfig(Mage_Core_Model_Config_Base $config)
    {
        $spaceId = Mage::getModel('core/website')->load(Mage::getSingleton('adminhtml/config_data')->getWebsite())
            ->getConfig('postfinancecheckout_payment/general/space_id');
        if ($spaceId) {
            $mergeModel = new Mage_Core_Model_Config_Base();
            $paymentMethodTemplate = file_get_contents(Mage::getModuleDir('etc', 'PostFinanceCheckout_Payment') . DS . 'payment_method.system.xml.tpl');
            foreach (Mage::getModel('postfinancecheckout_payment/entity_paymentMethodConfiguration')->getCollection()
    ...
    public function initConfigValues()
    {
        ...
        foreach (Mage::app()->getWebsites() as $website) {
            $websiteMap[$website->getConfig('postfinancecheckout_payment/general/space_id')][] = $website;
        }
        ...
        foreach ($collection as $paymentMethod) {
            ...
            if (isset($websiteMap[$paymentMethod->getSpaceId()])) {
                $basePath = '/payment/postfinancecheckout_payment_' . $paymentMethod->getId() . '/';
                $active = $paymentMethod->getState() == PostFinanceCheckout_Payment_Model_Entity_PaymentMethodConfiguration::STATE_ACTIVE ? 1 : 0;
                $model = 'postfinancecheckout_payment/paymentMethod' . $paymentMethod->getId();
     ...
     *
     * @param PostFinanceCheckout_Payment_Model_Entity_PaymentMethodConfiguration $paymentMethod
     * @param string $locale
     * @return string
     */
    private function getPaymentMethodTitle(PostFinanceCheckout_Payment_Model_Entity_PaymentMethodConfiguration $paymentMethod, $locale)
    {
    ...
    private function isTableExists()
    {
        ...
        if ($connection) {
            return $connection->isTableExists($resource->getTableName('postfinancecheckout_payment/payment_method_configuration'));
        } else {
            return false;
        }
    }
